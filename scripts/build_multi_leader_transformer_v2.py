#!/usr/bin/env python3
"""
Multi-Leader Transformer Decision Model  v2 — Political Context
===============================================================
Extends v1 by adding political leaders' actions as context features.
...
print("=" * 70)
print("MULTI-LEADER TRANSFORMER DECISION MODEL  v2 — POLITICAL CONTEXT")
print("=" * 70)
...
TOTAL_DIM = (N_ECON_FEATURES + LEADER_EMB_DIM + N_CONTEXT_FEATURES
             + N_POL_SLOTS * POL_EMB_DIM + N_POL_ACTION)
# = 10 + 16 + 5 + 24 + 5 = 60
...
def assemble_feature_vector(sample, cb_embeddings):
    ...
    return np.concatenate([econ_norm, cb_emb, ctx, pol_emb, pol_act]).astype(np.float32)
...
def main():
    ...
    # Step 8: Save model
    model_path = MODELS_DIR / "multi_leader_v2_political.pt"
    torch.save({
    ...
    print("\n" + "=" * 70)
    print("RESULTS SUMMARY  (v2 Political Context vs v1 Baseline)")
    print("=" * 70)

    print(f"\n  Architecture:")
    print(f"    v1: d_model=32, nhead=4, 2 layers, {TOTAL_DIM - N_POL_SLOTS*POL_EMB_DIM - N_POL_ACTION}D input")
    print(f"    v2: d_model=64, nhead=8, 3 layers, {TOTAL_DIM}D input  [{device}]")
    print(f"    GPU training time: {train_time:.1f}s")

    print(f"\n  Feature breakdown (v2):")
    print(f"    Economic state:       {N_ECON_FEATURES}D  (same as v1)")
    print(f"    CB leader embedding:  {LEADER_EMB_DIM}D  (same as v1)")
    print(f"    Context (streak):     {N_CONTEXT_FEATURES}D  (same as v1)")
    print(f"    US president:         {POL_EMB_DIM}D  NEW — Obama/Trump T1/Biden/Trump T2")
    print(f"    Xi Jinping:           {POL_EMB_DIM}D  NEW — active 2012-11+")
    print(f"    Putin:                {POL_EMB_DIM}D  NEW — active 2000-05+")
    print(f"    Political actions:    {N_POL_ACTION}D  NEW — trade/fiscal/VIX/sanctions/credit")
    ...
    eval_path = OUTPUT_DIR / "multi_leader_v2_evaluation.json"
